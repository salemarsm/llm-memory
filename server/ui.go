...
async function browserNav(path){const data=await api('/api/browse'+(path?'?path='+encodeURIComponent(path):''));document.getElementById('browserPath').textContent=data.path;document.getElementById('browserParent').value=data.parent;const list=document.getElementById('browserList');const entries=await Promise.all(data.entries.map(async e=>{let badge='';if(!e.is_dir){try{const st=await api('/api/ingest/status?path='+encodeURIComponent(e.path));badge=st.status==='unchanged'?' <span style="color:var(--accent);font-size:10px">✓ ok</span>':st.status==='changed'?' <span style="color:var(--warn);font-size:10px">⚡ changed</span>':st.status==='new'?' <span style="color:var(--muted);font-size:10px">new</span>':''}catch(err){}}return '<div style="padding:7px 10px;border-bottom:1px solid var(--line);cursor:pointer;display:flex;align-items:center;gap:8px" onclick=\''+( e.is_dir?'browserNav("'+e.path+'")':'browserSelect("'+e.path+'")'  )+'\''+'><span style="color:var(--muted)">'+(e.is_dir?'📁':'📄')+'</span><span>'+esc(e.name)+'</span>'+badge+(e.is_dir?'':'<span class="muted" style="margin-left:auto;font-size:11px">selecionar</span>')+'</div>'}));list.innerHTML=entries.join('')||'<div class="muted" style="padding:10px">pasta vazia</div>'}
...
async function loadSignals(){const qs=new URLSearchParams();const p=val('subjectFilter');if(p)qs.set('project',p);qs.set('status','*');qs.set('limit','200');const sigs=await api('/api/signals?'+qs.toString());const kindColor={notice:'var(--accent)',lease:'var(--warn)',handoff:'var(--blue)',conflict:'var(--bad)',review_request:'var(--warn)',blocker:'var(--bad)'};document.getElementById('rows').innerHTML='<tr><td colspan="8"><h3>Coordination Signals</h3><div style="margin-bottom:8px"><button onclick="newSignal()">Novo signal</button></div>'+(sigs.length===0?'<div class="muted" style="padding:10px">Nenhum signal.</div>':sigs.map(s=>'<div style="border-bottom:1px solid var(--line);padding:10px"><div class="row" style="align-items:center;gap:8px"><span style="color:'+(kindColor[s.kind]||\'var(--muted)')+'">'+esc(s.kind)+'</span><span class="pill">'+esc(s.status)+'</span><b>'+esc(s.owner_agent)+'</b>'+(s.target_agent?'→ <b>'+esc(s.target_agent)+'</b>':'')+'<span class="muted" style="margin-left:auto;font-size:11px">'+fmtDate(s.created_at)+(s.expires_at?' · exp '+fmtDate(s.expires_at):'')+'</span></div><pre style="margin:4px 0 6px">'+esc(s.payload)+'</pre>'+(s.topic_key?'<span class="muted" style="font-size:11px">topic: '+esc(s.topic_key)+'</span> ':'')+(s.memory_id?'<span class="muted" style="font-size:11px">mem: '+esc(s.memory_id)+'</span> ':'')+['acknowledged','resolved','cancelled'].filter(st=>s.status==='active'||(st==='resolved'&&s.status==='acknowledged')).map(st=>'<button class="secondary" onclick=\'updateSignal("'+s.id+'","'+st+'")\'> '+st+'</button>').join(' ')+'</div>').join(''))+'</td></tr>'}
async function updateSignal(id,status){try{await api('/api/signals/'+id+'/'+status,{method:'POST'});loadSignals()}catch(e){alert('Erro: '+e.message)}}
function newSignal(){const kind=prompt('Tipo (notice|lease|handoff|conflict|review_request|blocker):','notice');if(!kind)return;const payload=prompt('Descrição:','');if(!payload)return;const owner=prompt('Agent (ex: claude-code):','claude-code');const expires=kind==='lease'?prompt('expires_at (RFC3339, obrigatório para lease):',''):null;const sig={kind,payload,owner_agent:owner,project:val('subjectFilter')||''};if(expires)sig.expires_at=expires;api('/api/signals',{method:'POST',body:JSON.stringify(sig)}).then(()=>loadSignals()).catch(e=>alert('Erro: '+e.message))}
